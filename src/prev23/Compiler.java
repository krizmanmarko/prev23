...
public class Compiler {
	...
	private static final String phases = "none|lexan|synan|abstr|seman|memory|imcgen|imclin|asmgen|livean|regall|srcgen";
		...
		try {
			...
			while (true) {
				...
				if (Compiler.cmdLineArgValue("--target-phase").equals("regall"))
					break;
				try (SrcGen srcgen = new SrcGen()) {
					srcgen.finish();
				}
				if (Compiler.cmdLineArgValue("--target-phase").equals("srcgen"))
					break;
			}
